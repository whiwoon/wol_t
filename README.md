...
- 텔레그램 메시지로 원격 WOL 패킷 전송
- 지정된 1명의 사용자만 허용 (허가되지 않은 접근 차단 및 로깅)
- systemd 서비스 자동 등록 / 제거 (스크립트 자체가 설치·삭제 담당)
...
모든 수신 메시지의 `from.id`를 `ALLOWED_USER_ID`와 비교합니다.  
일치하지 않으면 거부 메시지를 보내고 경고 로그를 남깁니다.
